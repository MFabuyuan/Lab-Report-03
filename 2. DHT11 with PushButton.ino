...
This code displays temperature and humidity values measured from a DHT11 sensor on LCD screen. A pushbutton is also connected to alternate between showing the values of temperature in both Celsius and Fahrenheit and humidity.
*/
...
void loop() {
 ...
 if (PBcount == 4 ) {
  ...
  else {   // Display Temperature value on LCD (Celsius) with degree symbol
    lcd.setCursor(0,0);
    lcd.print(F("Temp: "));
  ...
  else {   // Display Humidity value on LCD 
    lcd.setCursor(0,0);
    lcd.print(F("Hum: "));
  ...
  else {   // Display Temperature value on LCD (Fahrenheit) with degree symbol 
